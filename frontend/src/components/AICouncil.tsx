...
function AgentCard({ agent }: { agent: AgentResult }) {
    ...
    return (
        <div className={`relative bg-gradient-to-br from-gray-50 to-white dark:from-slate-900 dark:to-slate-900 border border-gray-100 dark:border-white/5 rounded-2xl p-4 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg`}>
            {/* Header */}
            ...
            {agent.key_signals.length > 0 && (
                <div className="flex flex-wrap gap-1 mb-3">
                    {agent.key_signals.slice(0, 3).map((s, i) => (
            ...
            {expanded && (
                <p className="mt-2 text-[11px] text-gray-700 dark:text-slate-300 leading-relaxed border-t border-gray-100 dark:border-white/5 pt-2">
                    {agent.reasoning}
                </p>
            )}
        </div>
        ...
        <div className="flex flex-col space-y-5 w-full max-w-7xl mx-auto pb-4">
            ...
            {loading && (
                <div className="space-y-4">
                    {/* Chairman skeleton */}
                    <div className="h-40 bg-gray-100 dark:bg-white/5 rounded-2xl border border-gray-200 dark:border-white/5 animate-pulse" />
                    {/* Agent card skeletons */}
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                        {[...Array(5)].map((_, i) => (
                            <div key={i} className="h-32 bg-gray-100 dark:bg-white/5 rounded-2xl border border-gray-200 dark:border-white/5 animate-pulse" style={{ animationDelay: `${i * 100}ms` }} />
                        ))}
                    </div>
                </div>
            )}
